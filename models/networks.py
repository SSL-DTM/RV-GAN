...
config.MODEL.DISCRIMINATOR = 'NLayer'
config.MODEL.NUM_DISCRIMINATOR_LAYERS = 3
...
config.MODEL.EXTRA.STAGE1.NUM_CHANNELS = [64]
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, use_tanh=False):
    ...
    def forward(self, x):
        output_h, output_w = x.size(2), x.size(3)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
        ...
        x = F.interpolate(x, size=(output_h, output_w), mode='bilinear', align_corners=ALIGN_CORNERS)
        ...
        if os.path.isfile(pretrained):
            ...
            for k, v in pretrained_dict.items():
                if k.startswith('model.'):
                    if k[6:] in model_dict.keys() and model_dict[k[6:]].shape == pretrained_dict[k].shape:
                        update_dict[k[6:]] = v
                else:
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if len(gpu_ids) > 0:
        ...
        # net.to(gpu_ids[0])
        net = torch.nn.DataParallel(net, device_ids=gpu_ids).cuda()  # multi-GPUs
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'unet_128_new':
        net = UnetGenerator(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'unet_256':
        net = UnetGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'hrnet':
        net = HighResolutionNet(input_nc, output_nc)
    elif netG == 'hrnetTanh':
        net = HighResolutionNet(input_nc, output_nc, use_tanh=True)
    else:
